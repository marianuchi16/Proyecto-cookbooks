...
uses Unit3, Unit8, Unit11, Unit13, Unit1;
...
begin
    if form13.ok and not (form3.ADOPEDIDO.Locate('ISBN',nombre,[])) then
        ...
        form3.ADOLIBRO.Open;
        form3.ADOLIBRO.Refresh;
        form9.DBGrid1.Refresh;
        form9.DBGrid3.Refresh;
  ...
  if form13.ok and not (form3.ADOPEDIDO.Locate('ISBN',nombre,[])) then
          begin
          form3.ADOLIBRO.Locate('ISBN',nombre,[]);
          form3.ADOlibro.edit;
          form3.ADOLIBRO.FieldByName('Eliminado').AsBoolean:= true;
          form3.ADOLIBRO.Post;
          form3.ADOLIBRO.Close;
...
begin
  if not(key in ['A'..'Z','ñ','Ñ',#08,#135,'a'..'z',#32]) then
    key := #0;
end;
